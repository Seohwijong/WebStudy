...
public class EmpDAO {
	...
	public void empInsert(EmpVO vo)
	{
		try
		{
			...
			// ----------							  ---------
			// ''가 자동으로 붙는다 => table명 , 컬럼명에는 사용하지 않는다
			// MyBatis => $ , #
	...
	public List<Integer> empGetMgrData()
	{
		...
		// 제네릭은 반드시 클래스형만 사용 (제네릭스 : 면접의 단골 (데이터형의 표준화))
		try
		{
			conn=db.getConnection();
...
//					 + "AND empno=?";
			String sql="SELECT empno,ename,job,mgr,TO_CHAR(hiredate,'YYYY-MM-DD'),TO_CHAR(sal,'L999,999'),NVL(comm,0),"
					+ "(SELECT dname FROM myDept md WHERE me.deptno=md.deptno ) dname,"
					+ "(SELECT loc FROM myDept md WHERE me.deptno=md.deptno ) loc,"
					+ "(SELECT grade FROM myGrade mg WHERE me.sal BETWEEN mg.losal AND mg.hisal) grade "
					+ "FROM myEmp me "
					+ "WHERE empno=?";
			ps=conn.prepareStatement(sql);
			// ?에 값을 채운다
