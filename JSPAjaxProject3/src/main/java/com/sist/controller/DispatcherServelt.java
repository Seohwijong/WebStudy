 ...
 * model 2 (JSP / Java) => 단점 : 복합성 , 장점 : 여러명이 동시에 개발 , 추가 , 확장성 , 재사용이 용이 => 유지보수
 * 
 * 						=> Controller에 집중이 된다
 * domain : Controller를 나눠서 (분산) 작업 => 스프링 5 ~ 6
 * 			기능별 서버를 나눠서 독립적으로 개발
 * 			MSA => Spring - Cloud
 * 			--------------------- CI/CD (git 기반 , 도커 , 쿠바네티스 , 젠킨스)
 *          DevOps (개발 / 운영)
 *          
 * MVC의 동작 구조
 * -------------
 * * Controller를 찾는 방법 : .do (URL 주소 설정)
 * 
 * 				*.do
 *              -(어떤 단어가 들어가도 상관 없다)
 *              board/list.do
 ...
 *              food/category.do
 *              
 * 사용자 요청 ------------------- Controller(DispathcherServlet) ----------- Model ----------- DAO
 * 								(Front Controller)
 * 										|
 ...
 * JSP => 자바 => JSP (MV)
 * ***JSP => 서블릿 => 자바 => 서블릿 => JSP (MVC) => 유지보수 , 대규모
 * ---------
 * 
 */
import java.util.*;
