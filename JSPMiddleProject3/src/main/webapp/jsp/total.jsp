...
<%--
		...
		표현식)
			...
			내장객체 (일반 JSP ==> 자바 / HTML 분리)
					...
					자바 / HTML혼합
				= requestScope => 
				  기존의 request + 추가 데이터 설정 => request.setAttribute("key",값)
				  request.getAttribute("key")
				  		|
				  ${requestScope.key} => ${key} => 생략가능
				= sessionScope =>
				  session에 저장된 값을 읽기
				  ${sessionScope.key} => ${key} => 생략가능 => 우선순위 : request => session
				  
				= param : ${param.key} => request.getParameter("key")
				= paramValues : ${paramValue.key} => request.getParameterValues()
			   ...
			   core : 제어문 , URL관련 , 변수 설정 (request.setAttribute())
			   		  ...
			   		  ----
			   		  	<c:choose>
			   		  		...
			   		  		<c:when test="">처리문장</c:when>
			   		  		<c:when test="">처리문장</c:when>
			   		  		<c:otherwise>default문장</c:otherwise>
			   		  	</c:choose>
			   		  	=> switch , 다중 if
			   		  반복문
			   		  	<c:forEach> => for
...
<%--
 ...
 <%
 	...
 	names.add("박문수");
 	names.add("강감찬");
 	// EL은 사용할 수 없다 (EL ${} => request , session에 저장해야 함)
