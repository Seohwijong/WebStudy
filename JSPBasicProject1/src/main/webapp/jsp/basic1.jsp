<%-- 		주석
		...
		-------------------------------------
			*** 웹 프로그램 : C / S => cloud (CI/CD) 지속적인통합 / 지속적인 배포
				client / server
				   |		|
				   			Java/Oracle/Spring/Sprin-boot
				   					| JDBC => MyBatis => JPA
		...
		4. JSP 주로 사용하는 위치
		   ...
		   ---- JSP가 점차 사라지는중
		    ...
		    ------
		      ...
		      동적페이지 : JSP / Servlet (한개의 파일 안에서 데이터만 변경)
		      -------- 정적 쿼리 / 동적 쿼리 
		5. JSP 문법
		   ***= 지시자
		   = 스크립트릿
		...
		26page
		  ...
		  1. 사용자 요청
		     브라우저 주소창만 이용이 가능
		     http://localhost:8800/프로젝트명/폴더/xxx.jsp
		     => 	-------------- 서버에 접근
		  2. 맨 마지막 파일
		  ...
		  3. servlet
		        |
		      init() : 생성자 대신 => 멤버변수 초기화
		        |
		      service() ===> 브라우저에 요청한 내용에 대한 처리 => 응합할 HTML작성
		          = doGet()
		          = doPost()
		        |
		     destory() ==> 파일 이동 / 새로고침 => 자동으로 메모리 해제 (gc())
