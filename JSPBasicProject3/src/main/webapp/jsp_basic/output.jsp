...
<%-- 
	...
	----------------------------------------------------------------
	218page 빈을 이용한 JSP파링 작성
	--------------------------
	  jsp 액션 태그
	  => 화면 이동 , 화면 연결 , 값 전송 , 객체 생성 , 멤버변수에 값 주입 , 멤버변수의 값을 익기
	  														<jps:getProperty>
	  										<jsp:setProperty>
	  							  <jsp:useBean>
	  		|		|	***<jsp:param>
	...
	--------------------------	
	   useBean : 메모리 할당
	   setProperty : setXxx() => set메소드 호출
	   getProperty : getXxx() => get메소드 호출
--%> 
<!DOCTYPE html>
<html>
<head>
...
<body>
	이름:<%=bean.getName() %><br>
	성별:<jsp:getProperty name="bean" property="name"/><br>
	부서:<%=bean.getDept() %><br>
	직위:<%=bean.getJob() %><br>
	연봉:<%=bean.getPay() %><br>
</body>
